...
//   - passwordParameter() 변경 "password" -> "password" 나중에 암호 파라미터 이름을 변경할 경우를 대비해서
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        return http
            ...
            .authorizeHttpRequests()
                // 정규표현식(regular expression)으로 패턴을 설정
                // 예) .*\.html
                .regexMatchers(".*\\.html").permitAll() // 정규표현식과 일치하는 요청에 대해서 인증을 검사하지 않는다
            ...
            .formLogin()
                ...
                .successForwardUrl("/auth/success")   // 로그인 성공 후 페이지 컨트롤러로 포워딩 설정
            ...
            .logout()
                ...
                .permitAll()
                .and()
            // 4) CSRF(Cross-Site Request Forger) 설정: 기본이 활성화된 상태
            // - 데이터 변경에 관련된 요청(POST, PUT, PATCH, DELETE)을 받을 때 CSRF 검증을 수행한다
            // 즉, 서버에서 발급한 유효한 CSRF토큰이 있을 경우에만 해당 요청을 처리한다
            // 만약 CSRF 토큰이 없거나 무효하다면 요청을 거절한다
            // - Server-Side Rendering 방식에서는 Thymeleaf가 CSRF 토큰을 HTML <form>에 자동 삽입했다
            // - 그러나 Client-Side Rendering 방식에서는 HTML 폼에 CSRF 토큰을 삽입할 방법이 없다
            // - 해결책?
            // 클라이언트가 CSRF 토큰을 사용할 수 있도록 서버에서 응답할 때 쿠키로 보내도록 설정한다
            // XSRF-TOKEN 이라는 이름의 쿠키로 클라이언트에게 전송된다
            // - 클라이언트에서는 데이터 변경에 관련된 요청을 할 때 마다
            // 폼 파라미터 값(_csrf)이나 요청 헤더(X-XSRF-TOKEN)로 CSRF 토큰 값을 서버에 보내야한다
            .csrf()
