...
// 페이지 컨트롤러의 request handler를 호출 할 때
// DispatcherServlet(프론트 컨트롤러)는 request handler가 요구하는 파라미터 값을 주입해 준다
// 이때 @LoginUser 애노테이션이 붙은 파라미터는 이 클래스가 처리한다
public class LoginUserArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public boolean supportsParameter(MethodParameter parameter) {
        ...
        return parameter.hasParameterAnnotation(LoginUser.class) && parameter.getParameterType().isAssignableFrom(Member.class);
    }

    @Override
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
        // supportsParameter()의 리턴 값이 true인 경우, 즉 이 클래스가 처리할 수 있는 파라미터인 경우
        // 이 메서드에서 그 값을 준비해서 DispatcherServlet에게 리턴해준다
